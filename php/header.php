...
<html>
	<head>
		<title>NoobStein &bull; <?php echo ucfirst(strtolower($actual_name)); ?></title>
	...
	<body>
		...
		<?php
				...
				$sql_select_funds = "SELECT fund, total_profits FROM account_balance LEFT JOIN accounts ON account_balance.id=accounts.id WHERE account_balance.id='" . $id_user . "'";
				$result = $conn->query($sql_select_funds);
				$row = $result->fetch_assoc();
				if ($row['fund'] <= 1000) {
					$nav.= "<li class='nav-secondary' title='Funds'>
							<a class='nav' class='funds_analysis.php'><div class='funds-text'>" . round($row['fund'], 0, PHP_ROUND_HALF_DOWN) . "$</div></a>
						</li>";
				} else {
					$nav .= "<div class='funds-text'>" . round(($row['fund']/1000), 0, PHP_ROUND_HALF_DOWN) . "k$</div></a>
						</li>";
				}
			...
			elseif($status == 2) 
			{
				...
				if ($row['fund'] <= 1000) {
					$nav.= "<li class='nav-secondary' title='Funds'>
							<a class='nav'><div class='funds-text'>" . round($row['fund'], 0, PHP_ROUND_HALF_DOWN) . "$</div></a>
						</li>";
				} else {
					$nav .= "<div class='funds-text'>" . round($row['fund']/1000, 0, PHP_ROUND_HALF_DOWN) . "k$</div></a>
						</li>";
